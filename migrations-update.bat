@echo off
REM Helper script to update database with Entity Framework migrations
REM Usage: migrations-update.bat <project> [context]
REM
REM Examples:
REM   migrations-update.bat api
REM   migrations-update.bat dashboard ApplicationDbContext
REM   migrations-update.bat dashboard AppDbContext
...
if /i "%PROJECT%"=="api" (
    ...
    set CONTEXT_ARG=
    echo INFO: Updating database for API project...
    ...
    echo INFO: Updating database for Dashboard project with context: %CONTEXT%
) else (
...
REM Navigate to project directory and update database
cd "%PROJECT_PATH%"

echo INFO: Executing: dotnet ef database update %CONTEXT_ARG%
echo.

dotnet ef database update %CONTEXT_ARG%
if %errorlevel% equ 0 (
